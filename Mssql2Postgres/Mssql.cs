...
{
    public class Mssql : ISql<SqlDataReader>
    {
        private readonly string _connectionString;
        public string Database => _activeConnection.Database;
        ...
        public IEnumerable<T> ExecuteReader<T>(string query, Func<SqlDataReader, T> parser)
        {
            using (var cmd = ActiveConnection.CreateCommand())
            {
                cmd.CommandText = query;
...
END";
        ...
        {
            var query = $@"select * from {Database}.INFORMATION_SCHEMA.COLUMNS where TABLE_NAME=N'{tableName}'";
        ...
        public TableInfo[] GetTablesList()
        {
            var query =
                $"select TABLE_SCHEMA,TABLE_NAME from {Database}.INFORMATION_SCHEMA.TABLES where TABLE_TYPE='BASE TABLE'";
            return ExecuteReader(query, reader => new TableInfo()
            {
                TABLE_SCHEMA = reader["TABLE_SCHEMA"]?.ToString()??"dbo",
                TABLE_NAME = reader["TABLE_NAME"]?.ToString(),
            }).ToArray();
        }
    }
